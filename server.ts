...
async function initializeStaticRoutes(
  ...
  try {
    ...
    if (VERBOSE) {
      if (loaded.length > 0 || skipped.length > 0) {
        ...
        allFiles.forEach((file) => {
          ...
          const reason = !isPreloaded && file.size > MAX_PRELOAD_BYTES
            ? "too large"
            : !isPreloaded
            ? "filtered"
            : "preloaded";
          const route = file.route.length > 30
            ? file.route.substring(0, 27) + "..."
            : file.route;
          console.log(
            `${status.padEnd(12)} │ ${route.padEnd(30)} │ ${file.type.padEnd(28)} │ ${
              reason.padEnd(10)
            }`,
          );
        });
    ...
    if (loaded.length > 0) {
      log.success(
        `Preloaded ${String(loaded.length)} files (${
          (totalPreloadedBytes / 1024 / 1024).toFixed(2)
        } MB) into memory`,
      );
    } else {
    ...
    if (skipped.length > 0) {
      ...
      log.info(
        `${String(skipped.length)} files will be served on-demand (${
          String(tooLarge)
